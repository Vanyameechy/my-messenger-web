...
db.exec(`
    ...
    CREATE INDEX idx_participants_user ON chat_participants(userId);
`);
...
function getUserById(id) {
    return db.prepare('SELECT id, username, displayName, avatar, bio, createdAt, settings FROM users WHERE id = ?').get(id);
}
...
// *** КЛЮЧЕВАЯ СТРОКА: раздача статических файлов из папки public ***
...
app.get('/api/chats', authenticate, (req, res) => {
    const chats = db.prepare(`
        SELECT c.*,
               (SELECT COUNT(*) FROM messages m WHERE m.chatId = c.id AND m.id > cp.lastReadMessageId) AS unreadCount,
    ...
    // Парсим JSON участников и добавляем информацию о последнем сообщении
...
app.get('/api/chats/:id', authenticate, (req, res) => {
    ...
    const participants = db.prepare(`
        SELECT u.id, u.username, u.displayName, u.avatar, cp.role, cp.joinedAt
...
// Обновить информацию о чате (только для групп и с правами админа/создателя)
...
app.post('/api/chats/:id/participants', authenticate, (req, res) => {
    ...
    // Проверяем права (creator или admin)
    const role = db.prepare('SELECT role FROM chat_participants WHERE chatId = ? AND userId = ?').get(chatId, req.userId);
    if (!role || (role.role !== 'creator' && role.role !== 'admin')) {
        return res.status(403).json({ error: 'Insufficient permissions' });
    }
...
app.delete('/api/chats/:id/participants/:userId', authenticate, (req, res) => {
    ...
    // Если удаляем другого - нужны права
...
app.put('/api/chats/:id/participants/:userId', authenticate, (req, res) => {
    ...
    // Проверяем права: только creator может назначать админов, админ может делать member
...
app.get('/api/chats/:id/messages', authenticate, (req, res) => {
    const chatId = req.params.id;
    const { limit = 50, before } = req.query; // before = id последнего полученного сообщения (для пагинации назад)
    ...
    const messages = db.prepare(query).all(...params).reverse(); // переворачиваем, чтобы шли в хронологическом порядке
    res.json(messages);
});
...
app.post('/api/messages', authenticate, (req, res) => {
    ...
    // Получаем полное сообщение для отправки через WebSocket
...
// Хранилище соединений: userId -> набор сокетов (чтобы поддерживать несколько вкладок)
...
wss.on('connection', (ws, req) => {
    ...
    // Подписка на комнаты (чаты) – при получении сообщения 'join'
